...
// ── Tipo para el banner de adopciones ─────────────────────────────────────────
type AdoptionBanner = {
  id: string;
  img_url: string | null;
  is_active: boolean;
...
export default function CampaignBanner() {
  ...
  const [adoptionBanner, setAdoptionBanner] = useState<AdoptionBanner | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data?.img_url) setAdoptionBanner(data);
      });
  }, []);

  if (!campaign && !adoptionBanner) return null;
    ...
    <section className="bg-[#ff3ca5] py-10 px-4 relative">
      ...
      {campaign && (
        <>
          ...
          <div className="max-w-3xl mx-auto bg-[#ffffff] rounded-xl px-6 shadow-md hover:shadow-2xl transition-shadow scale-100 hover:scale-[1.02]">
            {/* Header blanco */}
      ...
      {/* ── Banner de adopciones ───────────────────────────────────────────── */}
      {adoptionBanner?.img_url && (
        <>
          <div className="text-left mb-6 mt-16">
            <h2 className="text-2xl lg:text-4xl font-black text-[#ffffff] font-logo">
              Feria de adopciones
              <img
                src="https://res.cloudinary.com/dmtehcd5t/image/upload/v1779947669/michipaceno-icono_5_yluzqc.png"
                alt="gatito adopciones"
                className="inline-block w-8 h-8 ml-2"
              />
            </h2>
            <p className="text-gray-100 text-md font-semibold leading-relaxed">
              ¡Dale un hogar a un michito!
            </p>
          </div>
          <div className={`max-w-3xl mx-auto ${campaign ? "mt-10" : ""}`}>
            <div className="rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-shadow hover:scale-[1.02] scale-100">
              <img
                src={adoptionBanner.img_url}
                alt="Banner de adopciones"
