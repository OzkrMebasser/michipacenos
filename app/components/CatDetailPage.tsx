...
export default function CatDetailPage() {
  ...
  const handleFacebook = () => {
    const url = `${SITE_URL}/michi/${cat.id}`;
    window.open(
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-8 py-8 mt-[8rem]">
        ...
        {/* Card — sin scale-110 para que no tape el botón */}
        <div className="bg-white rounded-3xl overflow-hidden shadow-xl border border-gray-200 lg:flex lg:items-stretch hover:shadow-2xl transition">
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                {allPhotos.map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${i === currentPhoto ? "bg-white w-4" : "bg-white/50 w-2"}`}
                  />
                ))}
              </div>
            )}
              ...
              <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-2 px-3">
                {allPhotos.map((url, i) => (
                  <button
                    ...
                    onClick={() => setCurrentPhoto(i)}
                    title={`Foto ${i + 1}`}
                    className={`w-10 h-10 rounded-lg overflow-hidden border-2 transition flex-shrink-0 ${i === currentPhoto ? "border-white" : "border-transparent opacity-60 hover:opacity-100"}`}
                  >
          ...
          <div className="p-6 sm:p-8 space-y-6 lg:w-1/2 lg:overflow-y-auto">
            <div>
              ...
              <p className={`font-bold ${cat.sterilized ? "text-teal-600" : "text-gray-500"}`}>
                <span className="font-semibold">Esterilizado/a:</span>{" "}
                {cat.sterilized ? "✂️ Sí" : "✗ No"}
              </p>
            </div>
        ...
        <button
          ...
          className="mt-4 w-full bg-[#1877F2] hover:bg-[#1565d8] text-white font-bold py-3 rounded-2xl text-base transition-all hover:scale-[1.02] flex items-center justify-center gap-3"
        >
